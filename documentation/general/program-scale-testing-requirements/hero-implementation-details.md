...
When testing the smart switch we have to run a test to get the switch latency without running the traffic through the DPU and then get the total system latency with the understanding that each packets travel once through the NPU to reach the DPU, than it travel through the DPU and once more it will travel through the NPU after it leave the DPU. 
